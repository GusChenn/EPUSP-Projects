...
vetorPe = zeros(1,limSup);
vetorPec = zeros(1,limSup);
vetorEbN = zeros(1,limSup);

for EbN=limInf:1:limSup
    N = Eb/(10^(EbN/10));
...
f1 = figure;
plot(vetorEbN(limInf:limSup), vetorPe(limInf:limSup), ':', 'color', 'black');
hold on;
grid on;
plot(vetorEbN(limInf:limSup), vetorPec(limInf:limSup), '-.', 'color', 'red');
xlim([5 35]);